...
{    # Test SQL injection prevention in DAO layer
    try {
        ...
        is $malicious_user->email, "'; DROP TABLE users; --", 'SQL injection stored as literal text';
...
{    # Test XSS prevention in stored data
    ...
    like $user->name, qr/<script>/, 'XSS script stored as literal text (not executed)';
}
...
{    # Test special character handling
    ...
    is $user->name, "User with $special_chars", 'Special characters preserved correctly';
}
...
{    # Test Unicode handling
    ...
    is $unicode_user->name, 'José María 测试 العربية русский', 'Unicode text preserved correctly';
}
...
{    # Test null byte injection
    try {
        ...
        unlike $null_byte_user->email, qr/\x00/, 'Null bytes removed or handled';
    }
    catch ($e) {
    ...
    for my $email (@$invalid_emails) {
        try {
            ...
            if ($user) {
                ...
                is $user->email, $email, "Invalid email stored correctly: $email";
            }
        }
        catch ($e) {
            $rejected_count++;
            like $e, qr/email|invalid|format/, "Invalid email properly rejected: $email";
        }
    }
    
    # Either emails are validated (rejected_count > 0) or stored safely (valid_count > 0)
    ok ($valid_count > 0 || $rejected_count > 0), 'Email validation working correctly';
}
