...
{    # Step 1: Program Discovery
    ...
    # Create a teacher for the events
    my $teacher = $dao->create( User => {
        username => 'robotics.teacher',
        password => 'password123',
        email => 'robotics.teacher@afterschool.org',
        name => 'Alex Robotics',
        user_type => 'staff'
    });
    ...
    use DateTime;
    for my $week (0..9) {
        my $start_dt = DateTime->from_epoch(epoch => time() + 86400 * (7 + $week * 7) + 3600 * 15); # 3 PM each week
        my $end_dt = DateTime->from_epoch(epoch => time() + 86400 * (7 + $week * 7) + 3600 * 17); # 5 PM each week
        ...
        try {
            ...
            diag "Error creating event for week " . ($week + 1) . ": $e";
        }
    }
    ...
    for my $i (0..2) {
        ...
        try {
            Registry::DAO::Attendance->mark_attendance(
                $dao->db, $event->{id}, $child->family_id, 'present', $child->family_id, undef, $child->id
            );
            $attendance_count++;
        }
        catch ($e) {
            diag "Error marking attendance: $e";
        }
    }
    
    # Verify attendance records
    my $attendance_records = $dao->db->select('attendance_records', 'COUNT(*)', {
        student_id => $child->family_id,
...
{    # Step 12: Complete Journey Verification
    ...
    my $family_data = $dao->db->query(q{
        ...
        LEFT JOIN attendance_records ar ON fm.id = ar.family_member_id
    ...
    my $orphaned_records = $dao->db->query(q{
        ...
        LEFT JOIN family_members fm ON ar.family_member_id = fm.id 
        WHERE fm.id IS NULL
