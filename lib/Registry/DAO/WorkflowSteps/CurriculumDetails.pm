use 5.42.0;
# ABOUTME: Workflow step for defining program curriculum details (name, description, objectives).
# ABOUTME: Stores curriculum data in the workflow run for use by later steps.

use Object::Pad;

class Registry::DAO::WorkflowSteps::CurriculumDetails :isa(Registry::DAO::WorkflowStep) {

method process ($db, $form_data, $run = undef) {
    $run //= do { my $w = $self->workflow($db); $w->latest_run($db) };

    my @errors;
    push @errors, 'Program name is required'        unless $form_data->{name};
    push @errors, 'Program description is required'  unless $form_data->{description};

    return { errors => \@errors } if @errors;

    return {
        curriculum => {
            name               => $form_data->{name},
            description        => $form_data->{description},
            learning_objectives => $form_data->{learning_objectives} || '',
            materials_needed    => $form_data->{materials_needed}    || '',
            skills_developed    => $form_data->{skills_developed}    || '',
        },
    };
}

method prepare_template_data ($db, $run, $params = {}) {
    my $curriculum = $run->data->{curriculum} || {};
    return {
        program_type_name   => $run->data->{program_type_name} || 'Unknown',
        name                => $curriculum->{name}               || '',
        description         => $curriculum->{description}        || '',
        learning_objectives => $curriculum->{learning_objectives} || '',
        materials_needed    => $curriculum->{materials_needed}    || '',
        skills_developed    => $curriculum->{skills_developed}    || '',
    };
}

}
