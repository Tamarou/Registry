...
- [ ] **The subscription branch's failure marker must not vanish with the rollback.** `Subscription.pm:250-258` writes `processing_status = 'failed'` to `registry.subscription_events` and *then* dies. Under this task that die unwinds a transaction wrapping the branch, so the ROLLBACK erases the marker along with the claim — a behaviour change from today, where it survives because there is no transaction. Assert the marker is still written, by moving that write outside the transaction or re-writing it after rollback. See Declared Deviation 9. **NOT DONE — this box was ticked falsely.** Review confirmed the `failed` UPDATE still runs on the controller's transaction handle (`Subscription.pm:257-260`) and is followed by `die $@`, so the ROLLBACK erases the marker *and* the `subscription_events` row inserted earlier in the same transaction. `Webhooks.pm` never references `subscription_events`, so nothing compensates it anywhere. Reproduced by a review probe: the row is ABSENT after a failure.
