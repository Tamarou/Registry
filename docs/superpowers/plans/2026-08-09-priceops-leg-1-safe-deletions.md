...
Two stranded callers the spec's Leg 1 row does not name, found by grep:

- **The `seti_test` test consumers.** `t/controller/tenant-create-session.t:66`, `t/user-journeys/alex/01-acquire-tenant.t:82-84,199-207,331-348`, `t/user-journeys/alex/03-platform-billing.t:77,164-171,214-216`. Exactly the shape the spec's own stranded-caller rule warns about.
- **`templates/pricing-plan-creation/review-activate.html.ep:166-181`** reads the six discount keys the orphaned form writes. The spec names the form and the step class but not this reader.
...
**Modified:** `lib/Registry/Controller/Webhooks.pm`, `lib/Registry/DAO/Family.pm`, `lib/Registry/DAO/PricingPlan.pm` (comment only), `lib/Registry/DAO/WorkflowSteps/RequirementsRules.pm`, `lib/Registry/DAO/WorkflowSteps/TenantPayment.pm`, two `templates/pricing-plan-creation/` templates, `t/controller/payment-failures.t`, `t/dao/family.t`, `t/dao/pricing-plan-amount-cents.t`, `t/dao/tenant-payment-schema-isolation.t`, `t/controller/tenant-create-session.t`, two `t/user-journeys/alex/` files, `t/database/migration-verification.t`, `sql/sqitch.plan`, four `sql/verify/` scripts, `sql/test-schema.sql`.
...
- Modify: `t/database/migration-verification.t:44-47`
...
**Why this is first:** every later leg ships migrations, and today nothing proves a revert script works. `t/database/migration-verification.t:44-47` currently reads:
...
```perl
#!/usr/bin/env perl
# ABOUTME: Proves the tip sqitch change reverts cleanly: deploy, dump, revert one, redeploy, dump, diff.
...
# between the two dumps.  Everything else must match exactly.
sub dump_schema ($label) {
    my $out = "$dir/$label.sql";
    system( "$pg_dump --schema-only --no-owner --no-privileges"
...
ok length($before), 'schema dumped after full deploy';

$sqitch->run( 'sqitch', 'revert', '-t', $uri, '--to', '@^', '-y' );
$sqitch->run( 'sqitch', 'deploy', '-t', $uri );
my $after = dump_schema('after');

is $after, $before,
    'reverting and redeploying the tip change restores the schema exactly';
...
Expected: PASS. The tip is `refund-amounts-cents` and `sql/revert/refund-amounts-cents.sql` is well-formed.
...
Run: `carton exec prove -lv t/database/revert-round-trip.t`
Expected: FAIL — "reverting and redeploying the tip change restores the schema exactly" fails, because `revert_harness_canary` survives the redeploy and appears in the second dump.
...
In `t/database/migration-verification.t`, replace lines 44-47:
...
- Modify: `t/controller/payment-failures.t:3,23-29,93-119,120-133,135-168,170-218,220-274`
...
Three of its four subtests are installment-based and die once the DAOs are gone. The fourth is not. Keep only the fourth.

Delete:
- The ABOUTME at `:3`, replaced below.
- Lines `:23-24`:
  ```perl
  use Registry::DAO::PaymentSchedule;
...
- Lines `:26-29`:
  ...
  ```
- The `create_test_schedule` helper (`:93-119`) and the `post_webhook` helper (`:120-133`).
- The `'card decline'` subtest (`:135-168`).
- The `'duplicate webhook'` subtest (`:170-218`).

Keep the `'refund updates payment and enrollment status'` subtest (`:220-274`) unchanged — it touches only `registry.payments` and `enrollments`.

Rewrite the ABOUTME at `:2-3` from:
...
Expected: `lib/Registry/Client/Stripe.pm` and `lib/Registry/PriceOps/PricingPlan.pm` themselves, plus `t/dao/pricing-plan-amount-cents.t:11`.
...
```bash
git add -A lib/Registry t/dao/pricing-plan-amount-cents.t
...
- Modify: `t/dao/family.t:230-255`
- Modify: `lib/Registry/DAO/WorkflowSteps/RequirementsRules.pm:45-93`
- Modify: `templates/pricing-plan-creation/requirements-rules.html.ep:82-98,121-162,346,351-352`
- Modify: `templates/pricing-plan-creation/review-activate.html.ep:166-181`
...
In `t/dao/family.t`, delete the two assertions and any setup that exists only for them:
...
In `lib/Registry/DAO/WorkflowSteps/RequirementsRules.pm`, delete three blocks:

- Early bird: `:45-68`
- Family / group: `:70-76`
- Volume: `:78-93`

Everything above `:45` and below `:93` stays.

- [ ] **Step 4: Delete the discount fields from the form template**

In `templates/pricing-plan-creation/requirements-rules.html.ep`, delete:

- the early-bird field group at `:82-98`
- the family-discount field group at `:121-162`
- the JavaScript that toggles them, at `:346` and `:351-352`

- [ ] **Step 5: Delete the stranded reader in the review template**

In `templates/pricing-plan-creation/review-activate.html.ep`, delete `:166-181` — the block that displays `$reqs->{early_bird_enabled}`, `early_bird_discount`, `family_discount_enabled`, `family_discount_amount`, `family_discount_type`, and `min_children`. Once the form stops writing those keys, this block renders nothing but is a reader for data that no longer exists.

- [ ] **Step 6: Confirm no key survives without both a writer and a reader**
...
- [ ] **Step 7: Re-import the workflow and run the affected tests**

The step class changed, so the workflow definition in the database must be refreshed:

```bash
carton exec ./registry workflow import registry
```

Run: `STRIPE_SECRET_KEY=ci_placeholder_not_a_stripe_key carton exec prove -lv t/dao/family.t t/dao/pricing-plan.t`
Expected: PASS.
...
- Modify: `t/controller/tenant-create-session.t:65-66`
- Modify: `t/user-journeys/alex/01-acquire-tenant.t:5,82-84,199-207,331-348`
- Modify: `t/user-journeys/alex/03-platform-billing.t:5,77,164-171,205-216`
...
In `t/user-journeys/alex/01-acquire-tenant.t`, replace the POST at `:199-207`:
...
Make the matching edit at `t/user-journeys/alex/03-platform-billing.t:164-171`, dropping `setup_intent_id => 'seti_test_billing_' . $$,` and rewriting the comment the same way.

- [ ] **Step 7: Update the funnel-documentation comments**

`t/user-journeys/alex/01-acquire-tenant.t:82-84`:
...
In `t/controller/tenant-create-session.t`, delete `:65-66`:
...
- Modify: `t/dao/tenant-payment-schema-isolation.t:43,170-206`
...
-- foreign keys, so the two FKs are re-added explicitly against tenant tables.
DO $$
DECLARE
    s name;
BEGIN
...
Delete the whole subtest at `:170-206`, `'migration schedule-guard: blocks move when scheduled_payments reference target'`. It inserts into `registry.payment_schedules` (`:185-191`) and `registry.scheduled_payments` (`:195-201`) and asserts `like $guard_err, qr/pre-flight FAILED/`. The guard it tested lives in `sql/deploy/tenant-scoped-payments.sql` and remains deployed; there is simply no longer a table to trip it.
...
Run: `carton exec prove -lv t/database/`
Expected: PASS. `revert-round-trip.t` now grades `drop-installment-schedules`; if the revert script's column order or constraint names are wrong, this is where it shows.
...
**What is being retired:** `sql/deploy/unified-pricing-infrastructure.sql:133,138` seeds a plan named `'Registry Plus - $100/month + 1%'` with `plan_type`/`pricing_model` `'hybrid'`, `amount` `100.00`, and `pricing_configuration` `{"monthly_base": 100.00, "percentage": 0.01, "applies_to": "customer_payments"}`. Its relationship rows are `active` (visible in `sql/test-schema.sql:2388,2408`). The design has no hybrid plan type; leaving an active offer means a tenant could select a plan nothing can price.
...
-- leaves rows suspended for other reasons alone.
UPDATE registry.pricing_relationships pr
   SET status     = 'suspended',
       metadata   = pr.metadata
...
1 (harness) → 2 (installment code) → 6 (installment tables) → 7 (plan retirement)
                    ...
                    4 (discount surface, #296)       ├→ independent of 2/6/7,
                    5 (seti_test bypass)             ┘  run in listed order
```
...
- Tasks 3, 4 and 5 touch disjoint files from 2, 6 and 7. They are ordered by convention, not necessity.
