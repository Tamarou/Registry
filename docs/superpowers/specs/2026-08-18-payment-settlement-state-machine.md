...
The typed columns alone fix neither.

**Implemented — and the per-debt sequence this section first proposed does not work.** Tracing it before writing any code: the old key derives from the owed-children list and a `refund_seq` key derives from a counter, but *both* move whenever the debt grows, and both callers sent the accumulated **balance**. A lost response followed by a new demotion therefore sends 13000 against an 8000 debt either way. A counter is more legible and changes nothing.

What landed is **per-increment state**, in a `refund_increments` jsonb column: each debt increment carries its own `seq`, `cents`, `children` and `settled_at`. `record_capacity_obligation` appends the clamped delta; `unsettled_refund_increments` reports what still has to reach Stripe; `capacity_refund_key($seq)` names one increment and never moves; `settle_refund_increment` subtracts exactly the increment it settles and is idempotent through a `settled_at IS NULL` guard.

Refunding the **delta** is what makes the total sent equal the total owed. Per-increment settled state is what keeps a genuinely failed increment retryable — which delta-refunding alone would have lost, since the next attempt would only cover the newest increment.

Two things the implementation found that this section did not anticipate:

- **The increment must be clamped, not just the balance.** Clamping only `refund_owed_cents` against `payments_refund_owed_cents_check` lets the increments total more than the cart — and since the increments are what actually reach Stripe, that is a refund larger than the payment. Both now clamp against the same remaining headroom, so the increments sum to the balance by construction.
- **An unresolved `refund_manual_review` must hold the row in `refund_pending`.** The old code deleted that flag on discharge, for a mechanical reason that no longer exists. The money reason points the other way: refunding the children whose shares *could* be computed does not discharge a share that could not, and the runbook finds these rows by status.

Every method is a targeted UPDATE naming only the columns it changes, never `save()` — both because `save()` is a whole-row write from a possibly-stale object, and because the arithmetic must be atomic against a concurrent settlement. This is §2.3's shape, arrived at early because this section needed it.
