...
`stripe_account_id`, Leg 8's family-Customer column and Leg 9a's quote columns take that loop.
...
- **An earlier draft drew that conclusion for the test suite and not for the state machine,
  which is where it bites.** `Payment::refund` (`Payment.pm:455-482`) and `refund_async`
  (`:578-601`) never read `$refund->{status}`. They pick between `refunded` and
  `partially_refunded` off locally computed arithmetic — `$refund_cents >= $amount_cents` —
  and the only non-terminal exit is an exception. A `pending` Refund is a 200 response, so it
  takes the terminal branch: Registry writes a completed refund for money that has not moved.
  Stripe holds it *"until your Stripe balance becomes sufficient"*, and under direct charges
  that balance is the tenant's. If it is still short when the pending-refund window expires,
  Stripe fails the refund (`failure_reason: insufficient_funds`), returns the money to the
  tenant's balance, and emits **`refund.failed`** with the guidance that a human must arrange
  another way to pay the customer. Registry subscribes to no such event — Leg 3's list carries
  `charge.refunded`, the dispute events, `account.updated` and
  `application_fee.created`/`.refunded`, and `refund.failed`/`refund.updated` appear nowhere in
  this document. Leg 12's `ReconcilePayments` cannot cover it either: it scans
  `status IN ('pending','processing','failed')` and retrieves the PaymentIntent, so a row
  already written `refunded` is outside the scan and a PaymentIntent retrieve is not a refund
  retrieve. Net: the family drops, the admin approves, the refund silently never pays, and the
  database, the admin UI and the parent's dashboard all read `refunded` with nothing that ever
  corrects them. **So Leg 3 sets the row from Stripe's answer**: `succeeded` is terminal,
  `pending` writes `refund_pending` — the same non-terminal value Leg 0 already introduces for
  the crash window between COMMIT and the capacity refund, because it means the same thing on
  both paths, a refund initiated and not yet confirmed — and `refund.failed`/`refund.updated`,
  subscribed in the same leg, move it off. An earlier draft named the Leg 12 reconciler as a
  second way out; it is not one, and the next bullet says why. This is not exotic: Stripe's own
  Connect guidance for this shape is *"conduct a balance check before initiating a refund to
  make sure that the connected account has sufficient funds."* `requires_action` is deliberately
  **not** handled — Stripe documents it for Konbini, PromptPay, Boleto and bank transfers, none
  of which the Payment Element card path reaches — so the `pending` branch alone carries the fix.
- **`refund_pending` is a state with no reader, and the retracted clause above was the tell.**
  Two bullets and two decision entries introduce the value; nothing in this document scans for
  it. The reconciler named as the backstop reads
  `status IN ('pending','processing','failed')` — the list this document sets a hundred and
  fifty lines up — and `refund_pending` is not in it. `Job::ProcessRefunds` is not a reader
  either: it claims `enrollments.refund_status = 'pending'`, a different column on a different
  table. And `refund.failed`/`refund.updated` cover only the population where a Stripe Refund
  object exists. The row that strands hardest is the one where none does: Leg 0 commits
  `refund_pending`, then the post-COMMIT `create_refund` fails. **Any** post-COMMIT failure
  does it — a crash, a timeout, a 500, the process killed mid-deploy — and Stripe's redelivery
  does not heal it, because the dedup claim this milestone puts in the *same* transaction has
  already committed, so the retry is deduped away by design. The row sits in `refund_pending`
  indefinitely with the family's money still in the tenant's balance, and every dashboard
  reads it as a refund in flight.

  **The reader is `Job::ProcessRefunds`, and it lands with the job in Leg 3.** The job already
  polls, already loops tenant schemas, and already exists for exactly this class of stranded
  row; it gains a second claim over `payments.status = 'refund_pending'` with no recorded
  refund id. Legs 0, 1 and 2 ship the state before the sweep exists, which is a three-leg gap
  and an honest one — Leg 0 writes the manual clearance procedure into the runbook rather than
  pretending something automated is watching. It is a crash window on a capacity refund, not
  a routine path.

  **It must list before it re-issues.** The shortcut is to re-`POST` under the stable
  `refund:capacity:<payment_id>` key and let idempotency sort it out, and that is a double
  refund waiting on a clock. Stripe's wording is explicit: *"You can remove keys from the
  system automatically after they're at least 24 hours old. We generate a new request if a key
  is reused after the original is pruned."* A stranded row is found by a sweep, and a sweep
  that runs a day late — a wedged cron, a paused worker, a Monday morning — re-issues past the
  pruning window and takes a second refund out of the tenant's balance. So the job calls
  `GET /v1/refunds?payment_intent=<id>` under `Stripe-Account` first and reconciles against the
  answer: a `succeeded` Refund settles the row, a `failed` one writes `refund_failed`, a
  `pending` one is left alone, and an **empty list** — no Refund was ever created — is the
  crash window, the only case that re-issues. The key stays, because it makes the sub-24-hour
  retry free; it is simply not the thing that makes the operation safe.

  **Leg 12's reconciler deliberately does not grow a fourth scan value**, and this is written
  down so a later reader does not add one as an obvious omission. That job retrieves the
  *PaymentIntent* and writes down what Stripe says. A `refund_pending` row's PaymentIntent
  still reads `succeeded`, so including it would drive the row back to `completed` and erase
  the pending refund — worse than leaving it alone. A refund is reconciled against the Refund
  list, which is `ProcessRefunds`' job, not the intent, which is the reconciler's.
...
- **A poller only recovers money if something re-enqueues it, and nothing in this repository
  ...
  **Leg 12 edits `docker-entrypoint.sh` in the same commit as the job**, and the Leg 3 test
  ...
  branch, and every name in the scheduler branch resolves to a registered task. Those are two
  *additional* assertions on top of the `lib/`-wide one above, not a restatement of it — an
  earlier draft read as a restatement, which would have narrowed the test to
  `docker-entrypoint.sh` and dropped the only assertion that catches the next paragraph.

  **And "the `add_task` registry" is the wrong thing to read, because a grep for `add_task`
  passes on the defect production is running right now.** `grep -rn add_task lib/` returns five
  task names; a booted app's `$app->minion->tasks` returns four. The fifth is
  `workflow_executor`, defined at `Job/WorkflowExecutor.pm:12` inside a `sub register` that
  `Registry.pm:71-75` never calls — four jobs are registered there and that is not one of them.
  It is enqueued from two sites in the class and reached from four callers, so it is a live
  enqueue against an unregistered task, and production shows the result: 78 rows in
  `minion_jobs` with `task = 'workflow_executor'`, `state = 'failed'`, no worker, every one of
  them *"Job appears stuck in queue"*, accumulating one per boot since 2026-03-22. A class that
  *defines* a task is not an app that *installed* it, and a test that greps source cannot see
  the difference. So the test boots the application and reads `$app->minion->tasks`, and the
  spec says why in one clause naming `WorkflowExecutor.pm:12`, because this is the same shape as
  (71): a check written against the artifact that is easy to read rather than the one that is
  true.

  **That makes `workflow_executor` a named line item in Leg 3 rather than a discovery in it**,
  since the leg's own gate goes red against it on day one and Leg 3 is the leg with the hard
  deadline. It is *not* a blind `->register($self)`. Registering it switches on a
  self-perpetuating 60-second chain that has never run continuously, and
  `WorkflowExecutor.pm:70-78` re-enqueues on the **failure** path as well as the success path —
  once per Minion attempt, with `attempts => 3` carried into every child — so a chain that
  starts failing multiplies rather than repeats. Leg 3 picks one of two and states which:
  register it *and* delete the failure-path reschedule, or delete the dead enqueue path and its
  four callers. Either is small; leaving it open is what turns a red gate into a leg that
  cannot close. The 78 orphaned rows are an operator cleanup after that lands, not part of the
  leg — nothing in this milestone writes to production data.

  Two corrections to the paragraph above, while the subject is open. `attendance_check` does
  have an intended recurrence chain (`AttendanceCheck.pm` → `WorkflowExecutor`), dead solely
  because of the missing registration; "nothing re-arms it" is true of the *task* — the chain
  enqueues `workflow_executor`, not `attendance_check` — so the once-per-boot observation and
  its conclusion both stand, but the sweep is minting one orphan per boot rather than simply
  idling. And `domain_verification` belongs with `waitlist_expiration` and `process_waitlist` in
  the list of tasks with no sweep-level re-arm at all.
...
- **A payment can be partially refunded exactly once, and the second attempt dies.**
  ...
  the whole leave the row `partially_refunded` forever. Leg 3 owns the arithmetic because Leg 3
  rewrites the refund parameters anyway — the guard itself moves earlier, see the next
  paragraph: the guard becomes "refundable while
  ...
  **The guard rewrite is four legs too late: Leg 0 is what hits it first.** Leg 0's capacity
  refund commits `refund_pending` inside the transaction and then, after COMMIT, calls
  `Payment::refund` — whose opening line is the same `die … unless $status eq 'completed'`.
  The leg that introduces the non-terminal status is the leg the guard rejects, so the refund
  never reaches Stripe at all. The rewrite therefore ships in **Leg 0**, in the same commit
  that introduces `refund_pending`, and Leg 3 inherits a guard that already tolerates
  non-terminal statuses instead of authoring one. Both entry points move together —
  `refund_async` carries the identical line at `:572` — and the cost is zero, because the work
  was already budgeted four legs later. Two corrections travel with the move. **The column is
  `payments.status`**, and every bare `status => 'refund_pending'` in this document should be
  read that way; `enrollments.refund_status` is a different column on a different table, and
  the CHECK-widening bullet below adds `refund_pending` to *that* vocabulary for the drop
  path, not to this one. And nothing in the schema catches the two being confused:
  `registry.payments.status` is `character varying(50) … NOT NULL` with no CHECK at all
  (`sql/test-schema.sql:1178-1191`), so a junk status on the payments side is accepted
  silently. What makes this a sequencing defect rather than a money hole is that the failure
  is loud, deterministic, pre-Stripe, and walked into by Leg 0's own mandated crash test.
  ...
  **The widened list is four values, not three: `refund_pending` goes in with them**, for the
  refund Stripe accepted but has not paid (see the refund bullets above). A vocabulary
  argument that reasoned about this column at length and still shipped only terminal values is
...
- **Projecting the fee is not reversing it, and an earlier draft prescribed the first as the
  ...
  leaves the money exactly where the defect says it is. Nothing on the webhook path can reverse
  a fee: `refund_application_fee_for_tenant` has one call site in the whole tree,
  `Payment.pm:114` inside `_refund_connect_params`, whose only callers are `Payment::refund`
  and `refund_async`. A tenant-initiated Dashboard refund reaches none of them, and Stripe
  forbids the tenant from doing it themselves — *"an application fee can be refunded only by
  the application that created the charge"* — so Registry is the only party able to return that
  money and no leg does. Meanwhile the design creates a column that promises it will:
...
- **Leg 3a's version bump breaks every invoice handler, silently, and that is the argument
  ...
  for Stripe to retry. **Three of the five die with Leg 1's installment deletion, not two:**
...
- **Two corrections to the paragraph above, and the first inverts what Leg 3a has to do.**
  *(a) `Stripe-Version` is a request header and cannot break a webhook handler.*
  `Service/Stripe.pm:15` holds the pin and `:25-32` attaches it as `'Stripe-Version' => $api_version`
  on outbound calls; that governs the shape of responses to *Registry's own API requests*. All
  five readers consume webhook **event payloads** — `Webhooks.pm` decodes `$self->req->body`
  and hands `$event->{data}` down; `Subscription.pm:314-316,327-329` read
  `$event_data->{object}` before making any API call; `ScheduledPayment.pm:23,51` receive the
  same. Stripe: *"webhook endpoints either have a specific API version set or use the default
  API version of the Stripe account."* Payload shape is set by the **endpoint's own
  `api_version`**, which nothing in this repository and nothing in this document ever sets —
  `api_version`, `webhook_endpoints` and `enabled_events` return one field and one comment
  across `lib/ bin/ t/ docs/`, and zero hits in these 4,200 lines. So merging Leg 3a as
  written does not break the invoice handlers, and — the part that matters — **rewriting
  `Subscription.pm:316,329` to the basil path while the delivering endpoint still emits
  acacia-shaped payloads manufactures the exact silent-undef failure the rewrite is prescribed
  to prevent.** Leg 3a therefore bumps the *endpoint* first, by Stripe's documented
  cutover — create a second endpoint at `api_version: 2026-07-29.dahlia`, dual-run, flip,
  disable the old — and the reader rewrite lands with it, in that order. Leg 3's two new
  endpoints are created with the version stated explicitly rather than inheriting an account
  default that can move under the platform, because Leg 8's `invoice.paid` handler — the one
  that writes the settled `payments` row — is being authored against dahlia shapes. And the
  leg's test asserts the delivered `event.api_version` equals the pin;
  `t/stripe-live/service-version.t:19-36` makes one `create_customer` call and asserts a `cus_`
  id comes back, so it exercises the outbound header and cannot see this class of defect at all.
  *(b) The harm was overstated as "renewals stop being recorded."* They are recorded either
  way: `Subscription.pm:211-217` INSERTs the full `encode_json($event_data)` into
  `registry.subscription_events` **before** any handler dispatches. What is lost is
  `update_billing_status` writing `registry.tenants.billing_status` — a column written in five
  places and read in none, its only non-write occurrence the `%TENANT_COLUMNS` allowlist at
  `Tenant.pm:141`. Nothing branches on it, no money moves, no access is gated. The durable
  defect is the unowned endpoint-version step, not the write-only column.
...
1. **Route and page.** `$admin_only->get('/dashboard/disputes')` following the pattern at
   `Registry.pm:705-715`, a method on `Controller::AdminDashboard`, a template in
   `templates/admin_dashboard/`. Admin pages here are plain controller-to-template, not
   workflow-driven, so this is the small kind of change. **The bridge is `$admin_only`, not
   `$admin`, and an earlier draft named the wrong one** — it cited `Registry.pm:676-703`, which
   is the `require_role('admin','staff')` block this document declares a money bug three legs
   earlier when it moves drop approval off it. `Registry.pm:705-715` is a second `under('/admin')`
   at `require_role('admin')` carrying the comment *"so that staff cannot reach these routes
   even though staff can reach other `/admin/*` routes"*; it is what domain management already
   uses, and `admin_only` appeared nowhere in this document before now. `staff` is the live
   instructor role, not a vestige — `GenerateEvents.pm:222` and `Schedule.pm:224` select
   teachers by it — and `Controller::AdminDashboard` does no internal role gating, so the
   bridge choice is the entire boundary. This correction is unconditional: it applies to the
   list-view branch and the embedded branch alike, since both mount a route.
2. **AccountSession endpoint.** On the same `$admin_only` bridge. A short-lived session scoped
   to the requesting tenant's connected account. It is an authorization boundary: the session
   must be minted from the tenant on the session, never from a parameter — and, because
   `dispute_management` lets the holder *accept* a dispute, which is a one-click irreversible
   forfeit of the disputed amount plus Stripe's fee against the tenant's own balance under
   `losses_collector: stripe`, role is as much of that boundary as tenant is. This piece exists
   only in the embedded branch of the fork above.
...
**Round 13 broke that: six confirmed, nine refuted, and the number moved for the first time in
three rounds.** The prediction above was wrong in the useful direction. Two of the six are the
same self-consistency shape 11 and 12 produced — a verify sweep that omits Leg 0 one round after
(44) named that exact failure mode, and a dispute route mounted on the bridge Leg 3 moves drop
approval *off* — and both cost nothing. But four are not, and three of those are money: an
application fee the design promises to return and no leg can, a `pending` refund written to the
database as a completed one with no event subscribed that could correct it, and a capacity
re-check that counts its own enrollment and refunds a child who is properly enrolled. The
fourth inverted a leg: Leg 3a's stated cause for existing was a request header that cannot
reach the code it was said to break, and the remedy it prescribed would have created that
failure rather than prevented it. **What separates round 13 from 11 and 12 is not the count but
the direction of the reading**: those rounds re-read the document against itself, and this one
re-read it against Stripe's semantics — who may reverse a fee, what a `pending` refund does when
the window expires, which version governs a delivered payload. Each of the three money findings
is a place where the document had read Stripe's documentation for an adjacent question and
carried the answer one step too far. That is a class this document has not yet exhausted, and
...
**Round 15 is the first round whose findings cost nothing, and that is the signal, not the
count.** Eighteen raised, three confirmed, and all three — (69), (70), (71) — are corrections
to *where* work sits or *what* a check reads, not new work: a guard rewrite moved four legs
earlier, a state given the reader it never had, an assertion pointed at the schema where the
mistake lands. The estimate does not move. Two of the three are the same shape as round 14's:
(70) is a state this document invented and then failed to give an exit, and (71) is (50)'s own
remedy failing on the reflex (50) exists to diagnose. The third class this round names is
narrower and worth keeping: **a check that cannot go red reads exactly like a check that passes**,
and the only way to tell them apart is to ask what would have to be true for it to fail. That
question was never asked of the tenancy invariant across four rounds that cited it approvingly.

**Round 16 says the loop is still finding money, and it found it inside last round's own
fix.** Fifteen raised, five confirmed, ten refuted; every survivor was a minority verdict on
its own panel, which is the shape of a round near the end rather than a round with nothing
left. (72) is the round-14 class one turn tighter: not a fix that became a defect in a later
round, but a fix that removed the false positive and left the true positive **named two
sentences above it in the same entry**. (75) is genuinely new and the one worth remembering
past this milestone: **a deferral is a judgement about today's code, and a leg that changes the
code invalidates the deferral without touching the deferred file.** The drop-approval bug was
correctly out of scope when it was filed and is a money bug the moment Leg 3 lands, and nothing
in the out-of-scope section was structured to notice. (73) and (74) are the two known
recurrences — a gate that passes on the live defect, and Stripe documentation read for an
adjacent question — and their recurrence after being named in rounds 11 and 13 is the argument
for continuing rather than for stopping. (76) is bookkeeping, and it is in the list only
because four enumerations agreeing with each other is what a leg author will trust.
The estimate does not move: (72)'s lock and demotion are inside Leg 0's existing capacity
block, (75)'s deny guard is three lines in files Leg 3 already opens, and (73) and (76) are a
test's implementation and a migration Leg 8 was always going to need.

**Round 17 is the first round the panels returned empty, and the one finding written in anyway
is the reason the count is not the answer.** Ten MAJOR raised across seven lenses, ten refuted,
zero survivors — and the refutations are the strongest the loop has produced: one of them
disproved a predicted production-only failure by *running* the harness and showing the same
`die` fires in the ordinary controller test, and another traced a claimed missing transaction to
a rule stated 350 lines away that cites the exact method by line. Five lenses were correctly
empty. Two findings were killed on grounds that also happen to be evidence the document is
holding: the thing the reviewer said was missing was already there, twice.

The overridden kill folds into (75), and it is worth more than the other nine put together. A reviewer
found that (75)'s deny guard covered two of the six steps and let three others act on a denial;
the panel granted the mechanism and killed it on blast radius, correctly — the residue is a
silent, self-expiring waitlist row in a chain this milestone never reaches. What the panel did
not check, and what makes it real, is that **(75)'s prescription does not work at all**: an
early return advances the pointer in this framework, and `next_step` is transient, so "return
early unless approve" is not a control-flow shape Registry's workflow engine has. An entry
written one round ago to stop a deferral from becoming a money bug specified a guard that would
have shipped without stopping anything. That is round 14's class — the fix that becomes the
defect — arriving one round faster than before and landing on the entry that named the class.
**The lesson is narrower than "keep reviewing" and worth stating: a document may prescribe a
control-flow shape the framework does not have, and no amount of reading the document finds
...
quoting and half refusing — and it carries a tenant-schema column too, so "half-deployed" there
also means some tenants missing the column the new code writes. Leg 6's publish backfill remains the clearest case because it is
...
**Both are registered with `api_version` set, and until this round no leg owned that field at
all.** A webhook endpoint created without one inherits the account default, and the delivered
payload is rendered at *that* version regardless of what `Stripe-Version` the platform sends on
its own requests. Leg 3 is writing a `v2.core.account.*` parse path and Leg 8 an `invoice.paid`
handler that produces the settled `payments` row; both are being authored against
`2026-07-29.dahlia` shapes, so the version is stated explicitly rather than inherited.

**But the two endpoints are different object types and only one of them has the field, and an
earlier draft said "both."** The Connect endpoint is a v1 `POST /v1/webhook_endpoints`, which
does take `api_version` and whose create response populates `secret` — that is the endpoint
carrying `invoice.paid`, `charge.refunded`, the disputes and the application-fee events, so the
load-bearing half of the rule is right where it matters, and it keeps the pin. The platform
endpoint is a v2 `POST /v2/core/event_destinations` with `event_payload: thin`, and its
parameter list is `enabled_events`, `event_payload`, `name`, `type`, the two cloud-target
objects, `description`, `events_from`, `include`, `metadata`, `snapshot_api_version` and
`webhook_endpoint`. There is no `api_version`. `snapshot_api_version` is not a substitute —
it applies only to `event_payload: snapshot`. Nor is the stated rationale true there: a thin
payload is *unversioned by design*, so there is no account default for it to drift with. The
version that governs the shape Leg 3 actually parses is the `Stripe-Version` header on the
follow-up retrieve of the event and its related object, which `Service::Stripe` already sends
and Leg 3a pins. The v1 escape hatch is closed too, so a later round does not reopen it:
`POST /v1/webhook_endpoints`'s `enabled_events` enum contains no `v2.core.*` type.

**The parameter that endpoint does need is the one no leg mentions.** `webhook_endpoint.signing_secret`
is *only includable on creation* — a create without `include: ["webhook_endpoint.signing_secret"]`
returns `"signing_secret": null`, as Stripe's own create example shows, and it cannot be fetched
back through the API afterwards. It is still visible in Workbench, so this is recoverable by a
human rather than fatal, but Leg 3 is registering these endpoints from code and its multi-secret
verification path has to have the value. So the Leg 3 row carries `include`, and it carries the
fact that the two secrets arrive in **different response fields** — `secret` on the v1 endpoint,
`webhook_endpoint.signing_secret` on the v2 destination — which is why the registration step
cannot be written as one loop over two symmetrical create calls. And Leg 3a's assertion that the
delivered `event.api_version` equals the pin stays scoped to v1 endpoints: a thin event body has
no `api_version` field, so that assertion is unwritable against the v2 destination.

The existing platform endpoint is a separate problem and belongs to Leg 3a, which bumps it by
Stripe's dual-endpoint cutover ahead of rewriting the readers that depend on the new shape.
...
twice over.** Counting the current table: Legs 0, 1, 2, 3, 4, 6, 7, 8, 9a, 9b and 10 all change
...
problem: the additive half is 9a and the drops are 9b, deliberately a deploy apart. Leg 8 was
the last one missed, and it was missed because its column is buried mid-sentence in a row that
reads as pure application work — see the paragraph below. **Eleven of
...
**Leg 8's migration is the one this document kept forgetting, and it never names the table it
lands on.** Four places classified Leg 8 as migration-free — the count above, decision (35)'s
list of legs needing the tenant loop, the sentence under `stripe_account_id` that names only
Leg 3 and Leg 9a as taking the loop, and the half-deployed-state paragraph that calls Leg 8 a
pure code cutover — while two places assign it "a tenant-schema column holding one customer id
per connected account." Both cannot be true, and the column is the true one. The nearest
counter is real and stated here so a later round does not re-raise it: the tenant loop is
described as the milestone's default for every data change, not as a per-leg opt-in. But a
default that four enumerations override by omission is not doing the job, so Leg 8 is added to
each, and Leg 8's row now states the table rather than the property.

The table is `users`. `clone_schema` gives each tenant its own copy
(`sql/deploy/fix-clone-schema-identifier-quoting.sql:45` inserts the seed row into
`<tenant>.users`), so the tenant schema *is* the account boundary, and one column on the payer
row yields "one Customer per connected account" without a join table or an explicit account
key. That is a design property, not an accident, and it is the reason to prefer `users` over a
new `stripe_customers` table — but Leg 8 confirms that parents actually hold rows in the tenant
schemas they enroll against before relying on it, and says so in the leg. Either way the change
is a `sql/deploy/` change and takes the
`sql/deploy/payments-amount-cents.sql:30-54` loop like every other one, which also means
Leg 8's half-deployed state is not "half the enrollments quoting" alone: it is that plus a set
of tenants whose `users` table has no column for the id the code is writing.
...
- **Leg 0** replaces `enrollments_session_student_type_unique` with a partial unique index, and
  this leg was missing from the sweep for the same reason Leg 7 was: it is not shaped like a
  migration leg, so nobody greps it. The replacement has to be partial on active statuses, and
  Postgres can only express that as an index, so the constraint name leaves
  `information_schema.table_constraints` whatever the new object is called. Two verify scripts
  name it and **both `RAISE EXCEPTION` rather than going vacuous**:
  `flexible-enrollment-architecture.sql:8-18` asserts it exists, and
  `fix-multi-child-enrollments.sql:21-31` asserts one of it and
  `enrollments_session_family_member_unique` — and the alternate cannot save it, because
  `flexible-enrollment-architecture.sql:10` drops that one and `:37-48` asserts it is gone.
  Note where this does *not* fire: `sqitch.conf`'s `[deploy] verify = true` runs each change's
  verify at its own point in the plan, when the constraint still exists. Only the
  no-argument `sqitch verify` at `t/database/migration-verification.t:24-27,62-64` re-runs
  every script against the final schema, and that is a `prove -lr t/` file, so the leg turns a
  test red whose name gives no hint that pricing work broke it.
...
That is **sixteen distinct files and eighteen per-leg obligations** —
...
`sql/verify/stripe-subscription-integration.sql:9`" understates the obligation by fifteen
...
below. Leg 0's two are the exception that is stated rather than absorbed silently: they are
two constraint-name assertions against a migration the leg already ships, not five objects and
a view like Leg 7's, so they sit inside its existing 9-11 rather than adding to it.
...
**A third half, because the first two are blind to everything that is not a table.** The four
table names catch nothing about a stray function, and 212 copied `btree_gist` C functions in
every tenant schema is the trap decision 50 describes. But the obvious form of the assertion —
compare the tenant schema's `pg_proc` count against the `registry` baseline, fail on any excess
— **cannot fail, and an earlier draft specified exactly that.** `clone_schema`'s function loop
is unfiltered: `SELECT proname as func_name, oid as func_oid FROM pg_proc WHERE pronamespace =
src_oid` (`sql/test-schema.sql:350-363`), then `pg_get_functiondef`, `replace`, `EXECUTE`, for
every row it returns. Equal counts are an identity of the copy, not a property of correct
extension placement. With 212 stray functions in `registry`, all 212 are copied, both sides
read 218, and the assertion is green on precisely the state it exists to catch.

So it is made about the **source** schema, where the mistake actually lands: `registry`
contains zero functions in an untrusted language.

```sql
SELECT count(*) FROM pg_proc p JOIN pg_language l ON l.oid = p.prolang
 WHERE p.pronamespace = 'registry'::regnamespace AND NOT l.lanpltrusted;
```

That goes red the instant a bare `CREATE EXTENSION` lands C functions in `registry` — three
steps before `clone_schema` is called, which is where the fault is — and stays green for every
legitimate `plpgsql` function anyone adds later. `registry` holds six today, all `plpgsql`
(`sql/test-schema.sql:206, 444, 484, 671, 690, 706`), matching production exactly. The
tenant-side count stays, demoted to what it actually proves: that the clone ran at all.
**It is a population check and not a permission check because the test runs as superuser under
`Test::PostgreSQL`** — the copy that raises `permission denied for language c` in production
succeeds in CI — but the population it reads has to be the source side, because the tenant
side is a mirror by construction.
...
**A third, because those two are tenant-scoped and say nothing about role:** a session whose
`user_type` is `staff` gets 403 from both the disputes page and the AccountSession endpoint.
That test already exists in another form and runs green —
`t/controller/tenant-domains.t:108-110` sets a `staff` session and asserts 403 against the
same `$admin_only` bridge — so this is a copy, not a new pattern.
...
- **The drop-approval workflow bug.** `ProcessAdminDropDecision.pm:33` advances one step of
  six while `templates/admin-drop-approval/complete.html.ep:7` reports success. It gets its own
  issue now — and `ProcessAdminTransferDecision.pm:31` has the identical `new_run`-and-return
  shape and joins the same issue, since the transfer chain moves no money and stays out of
  Leg 3 entirely.

  **But the rationale an earlier draft gave — "an operator-trust bug, unrelated to pricing" —
  stops being true the moment Leg 3 lands, and this entry is the deferral that turns into a
  money bug.** Today the one-step advance is what makes the rest of the chain unreachable, so
  the chain's own defect is invisible: *no step in `drop-request-processing` reads `action`*.
  `grep -n action` over `ValidateDropRequest`, `ProcessEnrollmentDrop`, `ProcessDropRefund`,
  `ProcessWaitlistAfterDrop`, `NotifyDropProcessed` and `CompleteDropRequest` returns nothing
  in any of the six, while `ProcessAdminDropDecision.pm:21` puts `action => $action` into the
  child run's data and `:25` hardcodes `admin_approved => 1` for deny as well as approve.
  `ProcessEnrollmentDrop.pm:34` then writes `status => 'cancelled'` unconditionally and
  `CompleteDropRequest.pm:27,35` writes the literal `'approved'` unconditionally. So a chain
  that actually advanced would process a **denied** drop exactly as an approved one.

  Before Leg 3 that is bad but bloodless: `ProcessEnrollmentDrop.pm:38`'s
  `refund_status => $data->{refund_requested} ? 'pending' : 'none'` reads a key nothing sets,
  so the ternary always takes the `'none'` branch and no refund row exists. **Leg 3 is what
  sets that key** — propagating `refund_requested` out of `ValidateDropRequest` is one of its
  named deliverables — and Leg 3 also ships `Job::ProcessRefunds`, which claims `pending` rows
  and sends money with no further human step. After Leg 3, whoever picks up the deferred issue
  and fixes the advance as a UI bug ships a path where clicking **Deny** cancels the
...
Two more are perigrin's calls rather than a review's findings. The thirty after them (31-60)
...
30. **The bar moved from "PriceOps aligned" to "ready to take money."** Five findings sat
    ...
    **And the branch is only as good as the predicate that reaches it, which this decision also
    did not supply.** The only capacity primitive in the tree, `Enrollment->count_for_session`
    (`:213-223`), is `COUNT(*) … status IN ('active','pending')` with no self- or
    payment-exclusion — it is what the pre-checkout check uses at
    `MultiChildSessionSelection.pm:93-100`. But `finalize_enrollment` inserts this payment's own
    rows as `active` (`Payment.pm:350,360`), and it runs on **both** settlement paths — the
    parent-return callback at `WorkflowSteps/Payment.pm:267` and the webhook at
    `Webhooks.pm:142` — with `Enrollment.pm:95`'s named `ON CONFLICT` arbiter making the second
    call a no-op. The parent-return path is not the 3DS exception:
    `templates/summer-camp-registration/payment.html.ep:164-169` calls `confirmPayment` with a
    `return_url` and no `redirect: 'if_required'`, so ordinary cards redirect back too. So on
    the last seat of a session the second path counts its own committed row, reads "capacity is
    gone," and refunds a child who is legitimately enrolled — and because the arbiter also makes
    the `waitlisted` insert a no-op against the existing `active` row, the end state is an active
    enrollment plus a refund, which is a free seat rather than a waitlisted one. The same missing
    predicate produces the mirror error on whichever path settles first: a two-sibling cart
    passing `9 >= 10` and overselling. Hence the predicate in the Leg 0 row — exclude this
    payment's rows, run before `finalize_enrollment`, and add this payment's items for the
    session back in before comparing.

    **The predicate fixes the false positive and leaves the true positive intact, and this
    decision named the true positive two sentences ago without fixing it.** Correct the
    predicate and the genuine oversell still ends the same way: the second path concludes
    capacity really is gone, its compensating `waitlisted` insert goes through
    `create_for_payment`, and the arbiter swallows it against the `active` row the first path
    already committed. `refund_pending` commits, the refund goes out after COMMIT, and the
    child stays enrolled. Active enrollment plus refund is a free seat, and the arbiter is
    doing exactly what it was written to do — the comment above it
    (`Enrollment.pm:75-81`) says so: *"Only the payment replay is meant to be quiet; everything
    else must raise."* A demotion is not a payment replay, and a `status`-blind
    `DO NOTHING` on `(session_id, student_id, payment_id)` cannot tell them apart. So Leg 0
    specifies three things, not one:

    1. **The re-check gates both callers**, not the webhook alone. It runs immediately ahead of
       every `finalize_enrollment` — `WorkflowSteps/Payment.pm:267` as well as
       `Webhooks.pm:142`. The Leg 0 row's "on whichever path settles first" only makes sense
       under that reading, but the row also says "inside that same `FOR UPDATE` block," which
       reads webhook-only. Say which; it is both.
    2. **The compensating write demotes.** `ON CONFLICT … DO UPDATE SET status =
       EXCLUDED.status` for this call site, or an `UPDATE` with an `INSERT` only on zero rows.
       Writing it as a plain insert is wrong in all three shapes available: through
       `create_for_payment` the arbiter silences it; through `Enrollment->create` it raises
       against `enrollments_payment_dedup` (`sql/test-schema.sql:3488`) and rolls back the whole
       webhook transaction into a three-day poison retry with the money captured; and with
       `payment_id => undef` to dodge the arbiter it co-exists with the `active` row under Leg
       0's own status-aware partial index, so `count_for_session` counts the child twice.
    3. **The second path short-circuits when this payment already holds `active` rows for the
       session and the first path's check passed.** Without this, (2) alone converts "active
       plus refund" into "waitlisted plus refund" for a child who was legitimately enrolled —
       better than a free seat, still wrong. With it: demote and refund when the seat was never
       available, do nothing when it was.

    **And the re-check narrows the oversell window rather than closing it**, which is worth
    saying because the Leg 0 row's `FOR UPDATE` is on the *payment* row and serializes nothing
    about the session. Two concurrent last-seat captures both pass; so do the writers with no
    capacity check at all — `Enrollment->enroll_children` (`:103-121`, the free path) and the
    waitlist's `accept_offer` — and so does an admin lowering `sessions.capacity`. Closing it
    needs a session-scoped lock (`SELECT … FROM sessions WHERE id = ? FOR UPDATE`, or a
    transaction-scoped advisory lock keyed on `session_id`) taken before the count. Leg 0 takes
    the lock; it is one statement in a block that is already open, and without it the branch
    this decision has now specified three times is still reachable by the case it was written
    for. Neither existing test would catch any of it:
    `t/controller/payment-intent-webhook.t:106` and `t/integration/tenant-paid-enrollment.t:355`
    exercise both paths but never against a full session, so Leg 0's tests need one that does.
...
37. **The document is too long for what it is, and the length is kept deliberately — for
    ...
    items 5-60 of this list are a changelog rather than a set of standing decisions, and the
...
44. **The verify sweep missed the leg that drops a table without dropping a column.** (38)
    ...
    goes vacuous rather than red. The corrected count is **sixteen distinct files and eighteen
    per-leg obligations**, not thirteen; the sentence saying it understated the obligation by
    twelve files understated it by three more. **The rule (38) stated was right and the way it was
    applied was not: "grep `sql/verify/` for the name" has to run in every leg that drops
    anything, not in the legs a reviewer happens to think of as migration legs.** (This entry
    was filed at fourteen and sixteen, counting Leg 7. See (55): the next round found Leg 0
    missing from the same sweep, which is why the figures here read sixteen and eighteen.)
...
50. **`registry` is a schema `clone_schema` copies out of, so nothing belongs in it that a
    ...
    tenancy invariant test grows a third assertion — **on the `registry` side, not the tenant
    side**: zero functions in an untrusted language in `registry`. Comparing the two counts,
    which is what this entry said first, cannot fail; see (71).
...
55. **Leg 0 is the second leg the verify sweep missed, and (44) was the entry filed about the
    first.** Leg 0 replaces `enrollments_session_student_type_unique` with a partial unique
    index — which Postgres can only express as an index, so the constraint name leaves
    `information_schema.table_constraints` — and two deployed verify scripts assert that name
    with `RAISE EXCEPTION`. The alternate name in the second script's `IN` list is no escape:
    an earlier change drops it and asserts it gone. Nor does `[deploy] verify = true` catch it,
    because sqitch runs each verify at its own point in the plan, when the constraint still
    exists; only the no-argument `sqitch verify` in `t/database/migration-verification.t` re-runs
    every script against the final schema, and that file is in `prove -lr t/`. The count in (44)
    moves to sixteen files and eighteen obligations. **What makes this worth its own entry is
    that (44) named the failure mode exactly — "not in the legs a reviewer happens to think of
    as migration legs" — and the next round found another leg the same reviewers had not thought
    of.** A rule stated is not a rule applied; the sweep is only correct when it is re-derived
    from the drops, not from the legs that look like migrations. Costed at zero: two
    constraint-name assertions against a migration Leg 0 already ships.

56. **Projecting a fee is not reversing it.** The `dashboard: full` bullet named "the
    application fee is never reversed" as the defect and prescribed handlers that *record*
    `application_fee_amount` as the remedy — recording the loss the sentence above it had just
    identified. `refund_application_fee_for_tenant` has exactly one call site in the tree, inside
    `_refund_connect_params`, reachable only from `Payment::refund`/`refund_async`; a
    tenant-initiated Dashboard refund reaches none of them, and Stripe forbids the tenant from
    reversing the fee themselves. So Registry is the only party that can and no leg did, while
    `refund_application_fee NOT NULL DEFAULT true` promises otherwise for every tenant. Leg 3's
    handler now acts. Small per event, invisible by construction — it accrues *to* Registry, so
    the party losing money is the one who cannot see it.

57. **A `pending` refund is not a refund, and the document said so about the test suite only.**
    The direct-charge bullet already recorded that a refund can come back `pending` against a
    thin connected-account balance, and drew from it exactly one conclusion: don't assert
    `succeeded` in the re-cut suite. The state machine kept writing terminal. `Payment::refund`
    picks `refunded`/`partially_refunded` off local arithmetic and never reads
    `$refund->{status}`, so a 200 with `pending` becomes a completed refund in the database,
    and no leg subscribes `refund.failed` — the event Stripe emits when the pending window
    expires and a human has to arrange another way to pay. The non-terminal value is
    `refund_pending`, which Leg 0 already introduces for the crash window and which means the
    same thing here, so this adds a subscription and a status read rather than a vocabulary.
    **The tell was in the reasoning, not the code**: the passage that widens
    `enrollments_refund_status_check` argues at length about the vocabulary and still shipped
    only terminal values.

58. **The dispute surface was mounted on the bridge this document calls a money bug.** Leg 12
    specified `$admin->get('/dashboard/disputes')` and cited `Registry.pm:676-703` as the
    pattern — the `require_role('admin','staff')` block that Leg 3 moves drop approval *off*,
    three legs earlier, for exactly this reason. `Registry.pm:705-715` is a second
    `under('/admin')` at `require_role('admin')`, carrying a comment explaining that it exists
    so staff cannot reach it; it is what domain management uses and `admin_only` appeared
    nowhere in this document. The two dispute security tests were both tenant-scoped, so the
    defect would have shipped green. The route correction is unconditional across both branches
    of the Leg 12 fork; the AccountSession half — where `dispute_management` lets the holder
    *accept* a dispute — exists only in the embedded branch. **Citing a line range is not the
    same as reading it**, and this document cited one it had already condemned.

59. **`Stripe-Version` is a request header, so Leg 3a's stated cause was wrong and its
    prescribed fix was backwards.** The leg argued that pinning dahlia "breaks every invoice
    handler, silently," and therefore the `invoice.subscription` readers must be rewritten in
    the same commit. But the pin rides on Registry's outbound requests, and all five readers
    consume webhook payloads, whose shape is set by the **endpoint's** `api_version` — a field
    nothing in the repository and nothing in these 4,200 lines had ever set. So the bump alone
    breaks nothing, and rewriting the readers to the basil path while the endpoint still emits
    acacia payloads *manufactures* the silent-undef failure the rewrite was prescribed to
    prevent. Leg 3a bumps the endpoint first, by Stripe's dual-endpoint cutover, and Leg 3
    states `api_version` explicitly on both new endpoints rather than inheriting an account
    default that can move under the platform — which Leg 8's `invoice.paid` handler depends on.
    Two corrections travel with it: the harm was never "renewals stop being recorded"
    (`subscription_events` is written before dispatch) but a write-only `billing_status`
    column, and the existing live test asserts the outbound header and structurally cannot see
    this class of defect. **The document had read the right documentation for the wrong
    channel.**

60. **A capacity re-check with no stated predicate re-checks the wrong thing.** (30) added the
    re-check and (53) fixed its transaction ordering; neither said what to count. The only
    primitive in the tree counts `active`/`pending` rows for the session with no self- or
    payment-exclusion, and this payment's own rows are usually already `active` by the time the
    webhook runs, because `finalize_enrollment` runs on both settlement paths and the ordinary
    card redirects the parent back before Stripe's webhook lands. So a naive re-check counts its
    own row, reads "capacity is gone," and refunds a correctly-enrolled child — on the last seat
    of a full session, which is the popular session on the tenant's best day. The `ON CONFLICT`
    arbiter then makes the compensating `waitlisted` insert a no-op, so the end state is an
    active enrollment plus a refund: a free seat. The same silence produces the mirror error on
    ...
    specified the question it asks. See (72): this entry then fixed only the predicate, and the
    free seat it names in its own third sentence survives the fix.
...
69. **Leg 0 writes a status Leg 0's own next call rejects.** The capacity refund commits
    ...
    `die "Cannot refund non-completed payment" unless $status eq 'completed'` (`:449`;
    `refund_async` carries the same line at `:572`). The rewrite that tolerates non-terminal
    statuses was assigned to Leg 3 — four legs after the order 1, 0, 2, 3a, 3 puts the first
    write — so for that whole interval the capacity refund cannot reach Stripe. The rewrite
    moves into Leg 0, into the commit that introduces the state, and Leg 3 inherits it. Zero
    cost: the work was already budgeted, just late. Two things travel with it. The column is
    **`payments.status`**, stated explicitly, because `enrollments.refund_status` is a
    different column on a different table and (57) adds `refund_pending` to *that* vocabulary
    for the drop path. And no constraint would have caught the confusion —
    `registry.payments.status` is `varchar(50) NOT NULL` with no CHECK
    (`sql/test-schema.sql:1178-1191`) — so the `die` being moved is the only check there is.
    This is a sequencing defect and not a money hole because the failure is loud,
    deterministic, before Stripe is called, and inside the crash test Leg 0 already mandates.

70. **`refund_pending` had no reader for twelve legs.** Two bullets and two entries introduce
    the value and nothing scans for it: the Leg 12 reconciler reads
    `status IN ('pending','processing','failed')`, `ProcessRefunds` claims
    `enrollments.refund_status`, and `refund.failed`/`refund.updated` only fire where a Stripe
    Refund exists. The uncovered population is the one this document created — Leg 0's window
    between COMMIT and `create_refund`, where **any** post-COMMIT failure strands the row and
    Stripe's redelivery cannot heal it, because the dedup claim committed with the transaction
    and deduplicates the retry away. The reader is `ProcessRefunds`, in Leg 3, with a second
    claim over `payments.status = 'refund_pending'` and no recorded refund id; Legs 0-2 get a
    runbook line rather than a pretence that something automated is watching. **The job lists
    before it re-issues**: `GET /v1/refunds?payment_intent=…` under `Stripe-Account`, and only
    an empty list re-issues. Re-`POST`ing under the stable `refund:capacity:<payment_id>` key
    and trusting idempotency is a double refund on a clock — Stripe prunes keys after 24 hours
    and *"generate[s] a new request if a key is reused after the original is pruned"*, and a
    sweep is exactly the thing that runs a day late. Leg 12's reconciler stays out of it
    deliberately, written down so nobody adds the fourth scan value later: it retrieves the
    PaymentIntent, which still reads `succeeded`, and would drive the row back to `completed`.

71. **The tenancy invariant's third assertion could not fail.** It compared the tenant schema's
    `pg_proc` count against the `registry` baseline and failed on any excess. `clone_schema`'s
    function loop is unfiltered (`sql/test-schema.sql:350-363`), so equal counts are an
    identity of the copy, not a property of anything: with 212 `btree_gist` C functions in
    `registry`, all 212 are copied and both sides read 218. The assertion moves to the source
    schema — zero untrusted-language functions in `registry`, one `JOIN pg_language … WHERE NOT
    l.lanpltrusted` — which is red the moment a bare `CREATE EXTENSION` lands, three steps
    before `clone_schema` runs, and green for the six `plpgsql` functions `registry` legitimately
    holds. The tenant-side count is kept but demoted to evidence the clone ran. **This is (50)'s
    own fix failing on the same reflex (50) diagnoses**: the entry that says nobody authoring an
    extension line thinks of themselves as adding functions then wrote a check on the schema
    receiving them instead of the schema producing them.

72. **(60) fixed the false positive and left the true positive it had just named.** The
    capacity re-check gained a self-excluding predicate so it would stop refunding a
    correctly-enrolled child; the reason the predicate was needed — that `finalize_enrollment`
    has usually already written this payment's `active` rows by the time the webhook re-checks
    — is also the reason the capacity-gone branch does not work. `create_for_payment`'s
    `ON CONFLICT (session_id, student_id, payment_id) … DO NOTHING`
    (`Enrollment.pm:95`) has an arbiter that matches those rows exactly, so a `waitlisted`
    insert is discarded and the child holds a free seat after the refund. Three things follow,
    all in Leg 0: the branch writes `DO UPDATE SET status = EXCLUDED.status`; both settlement
    callers run the gate, not just the webhook; and the gate takes a session-scoped lock,
    because the `FOR UPDATE` the leg already specifies is on the payment row and two payments
    racing for the last seat never contend on it. The arbiter itself is left alone — the
    comment above it (`Enrollment.pm:75-81`) says the quiet insert is meant for the payment
    replay and everything else must raise, which is correct; `DO UPDATE` is how a caller that
    *intends* to change the row says so.

73. **"Exists in the `add_task` registry" is a gate that passes on today's defect.** The Leg 3
    test was specified against a grep of `add_task`, which finds five task names in `lib/`.
    `Registry.pm:71-75` installs four; `workflow_executor` is defined at
    `Job/WorkflowExecutor.pm:12` in a `register` method nothing calls, while
    `schedule_next_run` (`:85-95`) enqueues that name with `attempts => 3`. A class that
    defines a task is not an app that installed it, so the assertion must boot `Registry` and
    read `$app->minion->tasks`. Resolving `workflow_executor` becomes a named Leg 3 item, and
    the obvious fix is wrong on its own: adding the `register` call arms a job whose catch
    block (`:70-78`) reschedules on the failure path, so a job that fails three times
    re-enqueues itself three times. Register **and** delete that reschedule, or delete the
    enqueue path. Two smaller corrections came out of the same reading and are folded in:
    `attendance_check` and `domain_verification` are both properly registered, contrary to an
    earlier claim, and the fix does not depend on anyone remembering to call `->register` for
    `ProcessRefunds` — `Registry.pm:71-75` is that block and the instruction points at it.

74. **Only one of the two webhook endpoints has an `api_version`, and the document said
    "both."** Verified against Stripe's create references rather than inferred: `POST
    /v2/core/event_destinations` takes `enabled_events`, `event_payload`, `name`, `type`, the
    cloud-target objects, `description`, `events_from`, `include`, `metadata`,
    `snapshot_api_version` and `webhook_endpoint`, and no `api_version`;
    `snapshot_api_version` applies only to `event_payload: snapshot`, and a thin payload is
    unversioned by design, so the stated rationale does not transfer either. It also needs
    `include: ["webhook_endpoint.signing_secret"]` on create — the documented response
    example shows `"signing_secret": null` without it, and the value is then unrecoverable
    through the API. The pin survives where it earns its keep: the money path rides the v1
    Connect endpoint, which does take `api_version` and does return `secret`, and
    `POST /v1/webhook_endpoints`'s `enabled_events` enum contains no `v2.core.*` type, so the
    split is forced rather than chosen. Leg 3a's `event.api_version` assertion stays scoped to
    v1 events for the same reason. **This is the round-13 defect again** — reading Stripe's
    documentation for an adjacent question and carrying the answer one step too far.

75. **A deferral that was safe becomes a money bug because of what Leg 3 adds.** The
    drop-approval advance bug was filed out of scope as "an operator-trust bug, unrelated to
    pricing." Neither half survives Leg 3. No step in `drop-request-processing` reads
    `action`, so the chain processes a denial as an approval; that is currently invisible only
    because the one-step advance makes the chain unreachable and because
    `ProcessEnrollmentDrop.pm:38`'s `refund_status` ternary reads a key nothing sets. Leg 3
    sets that key and ships `Job::ProcessRefunds`, after which fixing the advance as a UI bug
    ships **Deny** cancelling the enrollment and refunding the family with no human in the
    loop. Leg 3 does not fix the advance; it makes the chain safe to advance, and the deferred
    ...
    *current* step's id (`WorkflowRun.pm:120-122`), so a guarded step returning a bare hashref
    still moves the pointer on. And there is no skip to jump to: `next_step` is stripped as
    transient (`WorkflowRun.pm:81-84`) and routing is the `depends_on` linked list
    (`WorkflowRun.pm:180-186`), so a step cannot name a successor other than itself. Guarding
    two of six steps therefore leaves three acting on a denial — `ProcessDropRefund`,
    `ProcessWaitlistAfterDrop`, which offers the seat of an enrollment that was never dropped
    (`ProcessWaitlistAfterDrop.pm:13-18` reads `session_id` and nothing else), and
    `NotifyDropProcessed`, which tells the parent their drop went through.

    **So the guard moves to the entry instead of the steps.** A denial is not a run of
    "drop-request-processing," and the cheapest correct shape is not to start one:
    `ProcessAdminDropDecision` already computes `$action` and already branches on it for its
    own message text (`:41-43`), so on deny it writes `denied` to the drop request itself and
    never calls `new_run` (`:33`). One conditional, in the one place that holds the decision,
    instead of five guards in steps that each have to re-derive it. `admin_approved => 1` at
    `:25`, which is hardcoded for deny today, goes away with the branch that made it a lie.
    This is the round-14 class landing on this round's own entry: the fix from one round became
    the defect of the next, and it did so because the entry prescribed a control-flow shape
    without checking that the framework has it.

76. **Leg 8 was counted as migration-free in four places and assigned a column in two.** The
    column is real — the family Customer id — and it never had a table. It lands on `users`,
    which `clone_schema` gives each tenant its own copy of, so the tenant schema is the account
    boundary and one column on the payer row is one Customer per connected account; Leg 8
    confirms parents hold rows in the schemas they enroll against before relying on that. The
    counterargument is noted rather than dismissed: the per-tenant loop is stated as the
    milestone's default for every data change, so the omissions are arguably harmless. They are
